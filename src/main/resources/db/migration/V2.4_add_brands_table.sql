create table musinsa.brands
(
  id             bigint auto_increment comment '아이디'
        primary key,
  name           varchar(100)             not null comment '브랜드명',
  english_name   varchar(100)             not null comment '브랜드 영문명',
...
create index brands_english_name_fulltext_index
  on musinsa.brands (english_name);

create fulltext index brands_name_fulltext_index
    on musinsa.brands (name);

create table musinsa.brands_histories
(
  id             bigint       not null,
